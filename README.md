...
- IoT Hub accepts requests above its message traffic throttle for a limited time. However, continuous violation of the throttle leads to a [traffic shaping effect](https://learn.microsoft.com/en-us/azure/iot-hub/iot-hub-devguide-quotas-throttling#traffic-shaping), which causes the IoT Hub message queue to fill up, resulting in rejected requests and data loss. A correct IoT Edge TTL configration and sufficient disk space as store/forward mechanism in edge hub are necessary for effective IoT Edge build-in retry. However, if the configuration expires or the situation at edge made the configuration less ideal, the built-in retry mechanism may fail. In such cases, the resiliency solution can serve as a fallback option to ensure data recovery.
- During an IoT Hub failover, there is a risk of edge-to-cloud data loss if the IoT Edge device cannot establish a connection to the new active IoT Hub instance in a timely manner. The IoT Edge built-in retry feature will attempt to resend the messages that were not acknowledged by the IoT Hub. However, the success of recovery depends on the timing of the failover event and the recovery of the connection. In this case, the resiliency solution can be an additional option to guarantee the recovery of lost data.
- When the IoT Edge is restarted, the messages in the queue on the device will be lost. The IoT Edge built-in retry feature can help recover some lost data in this case, however the unsuccessful retry attempts can occur due to persistent network issues or other factors.
