namespace ReliableRelayModule.Service
{
    using System;
    using System.IO;
    ...
    public class InfluxDBRecorder : ITimeSeriesRecorder
    {
        ...
        {
            ORM orm = null;
            using (Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("ReliableRelayModule.Service.TelemetrySelection.json"))
            using (StreamReader reader = new StreamReader(stream))//fixed - error Value cannot be null. (Parameter 'stream')
            {
                string result = reader.ReadToEnd();

                orm = JsonConvert.DeserializeObject<ORM>(result, new JsonSerializerSettings // convert TeleSelection.json file content to ORM format
                {
            ...
            foreach (var db in orm.Databases)
            {
                ...
                var retentionPolicy = new InfluxRetentionPolicy()
                {
                    ...
                    DBName = db.Name,
                    Duration = TimeSpan.FromDays(Configuration.GetValue("INFLUX_RETENTION_IN_DAYS", 3)),
                    IsDefault = false
                };
            ...
            foreach (JObject entry in entries)
            {
                foreach (var db in databases)
                {
                    ...
                    foreach (var measurement in db.Measurements)
                    {
                        ...
                        if (!success)
                        {
                            Console.WriteLine($"Could not add data point {measurement.Name}");
                            Log.Warning("Could not add data point {measurementName} to influx", measurement.Name);
                            continue;
                        }
                        Log.Information("Adding {measurementName} to InfluxDB was successful", measurement.Name);
                        Console.WriteLine($"Adding {measurement.Name} to InfluxDB was successful");
                    }
                }
            }
        }
        ...
        private static bool TryGetTimestampFromJson(out DateTime utcTimestamp, JObject entry, Database db)
        {
            try
            {
                ...
                utcTimestamp = DateTimeOffset.FromUnixTimeMilliseconds(unixTimeStamp).UtcDateTime;//utcTimestamp type: System.DateTime
                return true;
            }
            catch (Exception ex)
            {
                Log.Error("Error: Could not get/convert event timestamp in Json: {entry} \n {exceptionMessage}", entry, ex.Message);
                Console.WriteLine($"Error: Could not get/convert event timestamp in Json: {entry}, {ex.Message}");
